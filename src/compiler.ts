...
function program() {
    ...
    emitter.emit(`(import "console" "logRoman" (func $logRoman (param i32)))`);
...
function simpleStatement() {
    ...
    else if (context.currentToken === 'writeRoman') {
        writeRomanStatement();
        emitter.emit(``);
    }
...
function writeRomanStatement() {
    match('writeRoman');
    match('(');
    let e = expression();
    emitter.emit(`call $logRoman`);
