...
function isRomanStart(s: string) {
    return s === '§';
}
...
function isSpecialChar(s: string) {
    return !isDigit(s) && !isAlpha(s) && !isWhitespace(s) && !isEOF(s) && !isRomanStart(s);
}
...
export function readToken() {
    ...
    if (isRomanStart(context.current)) {
        readNext();
        while (isAlpha(context.current)) {
            buffer += context.current;
            readNext();
        }
        console.debug('ROMAN', buffer);
        const value = romanToInt(buffer);
